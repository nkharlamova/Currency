...
def test_contact_us_get(admin_custom_client):
    response = admin_custom_client.get('/currency/contact-us/create/')
    assert response.status_code == 200


def test_contact_us_post_empty_data(admin_custom_client):
    response = admin_custom_client.post('/currency/contact-us/create/')
...
def test_contact_us_post_valid_data(admin_custom_client, mailoutbox):
    ...
    response = admin_custom_client.post('/currency/contact-us/create/', data=payload)
    assert response.status_code == 302
    assert response.url == '/currency/contacts/list/'

    assert len(mailoutbox) == 1
    assert mailoutbox[0].to == [settings.EMAIL_HOST_USER]
    assert mailoutbox[0].subject == 'Subject Example'
